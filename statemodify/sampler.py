"""This module is responsible for generating samples for various parameters using different sampling methods."""
...
def validate_modify_dict(
    ...
    for key in required_keys:
        if (key not in modify_dict) and (key == "names") and (fill is True):
            modify_dict["names"] = [
                f"variable_{index}" for index, i in enumerate(modify_dict["ids"])
            ]

        elif key not in modify_dict:
            raise KeyError(
                f"Missing the following key in user provided modify dictionary:  '{key}'"
            )

    # duplicate the bound definition for all variables
    modify_dict["bounds"] = [
        modify_dict["bounds"] for i in range(len(modify_dict["names"]))
    ]
...
def build_problem_dict(
    modify_dict: dict[str, list[Union[str, float]]], fill: bool = False
) -> dict:
    ...
    setup_dict = validate_modify_dict(modify_dict=setup_dict, fill=fill)
...
def generate_samples(
    ...
    if sampling_method == "LHS":
        return latin.sample(problem=problem_dict, N=n_samples, seed=seed_value)

    else:
        raise KeyError(
            "Selected sampling method is not currently supported. "
            "Please file a feature request here: "
            "https://github.com/IMMM-SFA/statemodify/issues"
        )


def generate_sample_iwr(
    n_samples: int = 1,
    sampling_method: str = "LHS",
    seed_value: Union[None, int] = None,
):
    ...
    problem_dict = {
        ...
        "bounds": [
            [
                param_dict["iwr_multiplier"]["Upper_Colorado"]["lower"],
                param_dict["iwr_multiplier"]["Upper_Colorado"]["upper"],
            ],
            [
                param_dict["iwr_multiplier"]["Gunnison"]["lower"],
                param_dict["iwr_multiplier"]["Gunnison"]["upper"],
            ],
            [
                param_dict["iwr_multiplier"]["San_Juan"]["lower"],
                param_dict["iwr_multiplier"]["San_Juan"]["upper"],
            ],
            [
                param_dict["iwr_multiplier"]["Yampa"]["lower"],
                param_dict["iwr_multiplier"]["Yampa"]["upper"],
            ],
            [
                param_dict["iwr_multiplier"]["White"]["lower"],
                param_dict["iwr_multiplier"]["White"]["upper"],
            ],
        ],
    }
...
def generate_sample_all_params(
    n_samples: int = 1,
    sampling_method: str = "LHS",
    seed_value: Union[None, int] = None,
):
    ...
    problem_dict = {
        ...
        "bounds": [
            ...
            [
                param_dict["trans_multiplier"]["Upper_Colorado"]["lower"],
                param_dict["trans_multiplier"]["Upper_Colorado"]["upper"],
            ],
            [
                param_dict["trans_multiplier"]["Gunnison"]["lower"],
                param_dict["trans_multiplier"]["Gunnison"]["upper"],
            ],
            [
                param_dict["trans_multiplier"]["San_Juan"]["lower"],
                param_dict["trans_multiplier"]["San_Juan"]["upper"],
            ],
            [
                param_dict["trans_multiplier"]["Yampa"]["lower"],
                param_dict["trans_multiplier"]["Yampa"]["upper"],
            ],
            [
                param_dict["trans_multiplier"]["White"]["lower"],
                param_dict["trans_multiplier"]["White"]["upper"],
            ],
            [
                param_dict["muni_multiplier"]["Upper_Colorado"]["lower"],
                param_dict["muni_multiplier"]["Upper_Colorado"]["upper"],
            ],
            [
                param_dict["muni_multiplier"]["Gunnison"]["lower"],
                param_dict["muni_multiplier"]["Gunnison"]["upper"],
            ],
            [
                param_dict["muni_multiplier"]["San_Juan"]["lower"],
                param_dict["muni_multiplier"]["San_Juan"]["upper"],
            ],
            [
                param_dict["muni_multiplier"]["Yampa"]["lower"],
                param_dict["muni_multiplier"]["Yampa"]["upper"],
            ],
            [
                param_dict["muni_multiplier"]["White"]["lower"],
                param_dict["muni_multiplier"]["White"]["upper"],
            ],
        ],
    }
