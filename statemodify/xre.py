"""This module is responsible for extracting data from .xre files and converting it into a Pandas DataFrame."""
...
def extract_xre_data(
    ...
    if input_file is None and basin_name is None:
        raise ValueError(
            "Either input_file or basin_name must be set.  If using template, set basin_name."
        )
    ...
    for i in range(len(all_data_xre)):
        row_id_data = (
            []
        )  # this will store the structure ID, account num, year, month and init storage
        row_id_data.extend(
            all_split_data_xre[i][0].split()
        )  # loading the data described above
        if (
            row_id_data and row_id_data[0] == structure_id
        ):  # if the structure is the one we're looking for
            row_detail_data = all_split_data_xre[i][1:]  # first grab the initial data
            out_data.append(
                row_id_data + row_detail_data
            )  # combine it with the rest of the data and append to out_data
    ...
    if write_parquet:
        output_parquet = os.path.join(
            output_directory, f"{structure_name}_xre_data.parquet"
        )
