"""This module is responsible for modifying StateMod Demand (.ddm) files based on specified criteria."""
...
def modify_single_ddm(
    ...
    template_file = utx.select_template_file(basin_name, template_file, extension="ddm")

    # read in data specification yaml
    data_specification_file = utx.select_data_specification_file(
        yaml_file=data_specification_file, extension="ddm"
    )
    ...
    template_df[file_spec.value_columns] = (
        template_df[file_spec.value_columns].round(0).astype(np.int64)
    )
...
def modify_ddm(
    ...
    Modifed using a Latin Hypercube Sample from the user.
    Samples are processed in parallel. Modification is targeted at ids to modify are specified in
    the `modify_dict` argument. The user must specify bounds by which the samples will be generated.
    ...
    if save_sample:
        sample_file = os.path.join(
            output_dir, f"ddm_{n_samples}-samples_scenario-{scenario}.npy"
        )
        np.save(sample_file, sample_array)

    # generate all files in parallel
    results = Parallel(n_jobs=n_jobs, backend="loky")(
        delayed(modify_single_ddm)(
