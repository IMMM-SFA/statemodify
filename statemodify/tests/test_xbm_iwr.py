...
class TestXbmIwr(unittest.TestCase):
    ...
    XBM_COMP_FULLPATH = pkg_resources.resource_filename(
        "statemodify", os.path.join("tests/data", XBM_COMP_FILE_NAME)
    )
    IWR_COMP_FILE_NAME = "cm2015B_S0_test.iwr"
    IWR_COMP_FULLPATH = pkg_resources.resource_filename(
        "statemodify", os.path.join("tests/data", IWR_COMP_FILE_NAME)
    )
    REALIZATION_DIR = pkg_resources.resource_filename(
        "statemodify", os.path.join("tests/data")
    )
    ...
    def test_modify_xbm_iwr_run(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual(
                xbm_comp_data,
                xbm_sim_data,
                msg="Simulated data for XBM file does not match what was expected.",
            )
            ...
            self.assertEqual(
                iwr_comp_data,
                iwr_sim_data,
                msg="Simulated data for IWR file does not match what was expected.",
            )
