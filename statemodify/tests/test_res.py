...
class TestXbmIwr(unittest.TestCase):
    COMP_FILE_NAME = "gm2015B_S0_test.res"
    COMP_FULLPATH = pkg_resources.resource_filename(
        "statemodify", os.path.join("tests/data", COMP_FILE_NAME)
    )
    ...
    def test_modify_xbm_iwr_run(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual(
                comp_data,
                sim_data,
                msg="Simulated data for RES file does not match what was expected.",
            )
