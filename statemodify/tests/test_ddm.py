...
class TestDdm(unittest.TestCase):
    VALID_MODIFY_DICT = {"ids": ["3600507", "3600603"], "bounds": [0.5, 1.0]}

    DDM_COMP_FILE_NAME = "cm2015B_S0_test.ddm"
    DDM_COMP_FULLPATH = pkg_resources.resource_filename(
        "statemodify", os.path.join("tests/data", DDM_COMP_FILE_NAME)
    )
    ...
    def test_modify_single_ddm_run(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            stm.modify_single_ddm(
                modify_dict=TestDdm.VALID_MODIFY_DICT,
                query_field="id",
            ...
            self.assertEqual(
                comp_data,
                sim_data,
                msg="Simulated data for DDM file does not match what was expected.",
            )
