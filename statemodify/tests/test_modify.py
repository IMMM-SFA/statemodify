...
class TestModify(unittest.TestCase):
    ...
    COMP_STR = "item   focus\n"
    COMP_ADJ_DF_ADD = pd.DataFrame({"a": [10.0, 20.1], "b": [-1.0, -1.9]})
    COMP_ADJ_DF_MULTIPLY = pd.DataFrame({"a": [10.0, 2.0], "b": [-1.0, -0.2]})
    FIELD_DICT = {"a": [], "b": [], "c": []}
    ...
    def test_set_alignment(self):
        ...
        with self.assertRaises(AssertionError):
            s = modify.set_alignment(value="test", n_spaces=0, align="center")

        # check for left padding (right alignment) of four on value; total length of eight
        s = modify.set_alignment(value="test", n_spaces=4, align="right")
        ...
        s = modify.set_alignment(value="test", n_spaces=4, align="left")
        ...
        s = modify.set_alignment(value="test", n_spaces=0, align="left")
        ...
        s = modify.set_alignment(value="test", n_spaces=-2, align="left")
    ...
    def test_add_zero_padding(self):
        ...
        s = modify.add_zero_padding(x="0.1", precision=2)
    ...
    def test_construct_outfile_name(self):
        ...
        f = modify.construct_outfile_name(
            template_file=Path("/my/output/directory") / "file.txt",
    ...
    def test_construct_data_string(self):
        ...
        data = modify.construct_data_string(
            df=pd.DataFrame({"a": ["item   "], "b": ["   focus  "]}),
            column_list=["a", "b"],
    ...
    def test_apply_adjustment_factor_add(self):
        ...
        data = modify.apply_adjustment_factor(
            data_df=pd.DataFrame({"a": [10, 20], "b": [-1, -2], "c": [0, 1]}),
            value_columns=["a", "b"],
            query_field="c",
            target_ids=[1],
            factor=0.1,
    ...
    def test_validate_bounds_both(self):
        ...
        with self.assertRaises(
            ValueError, msg="High and low bounds validation failure."
        ):
