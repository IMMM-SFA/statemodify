...
class TestEva(unittest.TestCase):
    VALID_MODIFY_DICT = {"ids": ["10001", "10004"], "bounds": [-0.5, 1.0]}

    EVA_COMP_FILE_NAME = "cm2015B_S0_test.eva"
    EVA_COMP_FULLPATH = pkg_resources.resource_filename(
        "statemodify", os.path.join("tests/data", EVA_COMP_FILE_NAME)
    )
    ...
    def test_modify_single_eva_run(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            stm.modify_single_eva(
                modify_dict=TestEva.VALID_MODIFY_DICT,
                query_field="id",
            ...
            self.assertEqual(
                comp_data,
                sim_data,
                msg="Simulated data for EVA file does not match what was expected.",
            )
    ...
    def test_modify_eva_run(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual(
                comp_data,
                sim_data,
                msg="Simulated data for EVA file does not match what was expected in parallel function.",
            )
